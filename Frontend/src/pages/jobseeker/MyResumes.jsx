      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        {!showUpload && (
          <button
            ...
            className="mb-4 sm:mb-6 bg-blue-500 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-blue-600 transition-colors flex items-center gap-2 font-medium text-sm sm:text-base"
          >
        ...
        {/* Upload Component */}
        {showUpload && (
          <div className="mb-6 sm:mb-8 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 sm:p-6">
            <ResumeUpload
              onUploadSuccess={() => {
                setShowUpload(false);
                fetchResumes();
            ...
            <button
              ...
              className="mt-4 text-slate-400 hover:text-white transition-colors text-sm sm:text-base"
            >
              Cancel
            </button>
          </div>
        ...
        {resumes.length === 0 ? (
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-8 sm:p-12 text-center">
            ...
            <h3 className="text-lg sm:text-xl font-semibold mb-2">No resumes yet</h3>
            <p className="text-slate-400 mb-6 text-sm sm:text-base">Upload your first resume to get started</p>
            {!showUpload && (
              <button
                onClick={() => setShowUpload(true)}
                className="bg-blue-500 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-blue-600 transition-colors inline-flex items-center gap-2 text-sm sm:text-base"
              >
                <Upload className="w-4 h-4 sm:w-5 sm:h-5" />
                Upload Resume
              </button>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-500 to-purple-500 rounded-lg flex items-center justify-center mb-3">
                      <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                    </div>
                    <h3 className="text-base sm:text-lg font-semibold text-white mb-2 break-words group-hover:text-blue-400 transition-colors">
                      {resume.filename}
                    </h3>
                ...
                {resume.extractedTextPreview && (
                  <div className="mb-4 p-2 sm:p-3 bg-slate-900/50 rounded-lg text-xs sm:text-sm text-slate-400 line-clamp-3">
                    {resume.extractedTextPreview}
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-blue-500 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
                  >
                  ...
                  <button
                    ...
                    className="px-3 sm:px-4 py-2 border border-red-500/50 text-red-400 rounded-lg hover:bg-red-500/10 transition-colors"
                  >
          ...
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 z-50">
            <div className="bg-slate-800 border border-slate-700 rounded-xl max-w-6xl w-full h-[95vh] sm:h-[90vh] flex flex-col">
              <div className="p-3 sm:p-4 border-b border-slate-700 flex justify-between items-center">
                <h2 className="text-lg sm:text-xl font-bold text-white pr-4 truncate">{selectedResume.filename}</h2>
                <button
